...
void CL_SystemInfoChanged( void ) {
...
#ifdef USE_CURL
	// compatibility with older versions of dl redirect
	Q_strncpyz(clc.sv_dlURL, Info_ValueForKey(systemInfo, "sv_wwwBaseURL"),
		sizeof(clc.sv_dlURL));
#endif
	...
	while ( s ) {
		...
		if (!Q_stricmp(key, "fs_game"))
		{
            ...
            // account for "" == "base" when doing comparisons
            ...
            // check if the server changed fs_game
			if( Q_stricmp( value, fsgame ) )
            {
                Com_DPrintf("Server changed fs game: %s -> %s\n", Cvar_VariableString("fs_game"), value );
				cls.serverChangedFSGame = qtrue;
            }
		...
		else if(!Q_stricmp(key, "sv_wwwDownload")
			|| !Q_stricmp(key, "sv_wwwBaseURL"))
			; // ignore these for compatibility
		else
		{
