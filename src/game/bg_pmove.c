...
PM_CheckJump(void)
{
  ...
  {
    //pm->ps->velocity[2] += pm->ps->stats[STAT_JUMPSPEED];WTF BRAH
    pml.groundPlane = qfalse; // jumping away
    pml.walking = qfalse;
  }
...
PM_Weapon(void)
{
  ...
  { // hack to give pistol initial ammo, i know it isnt the right way to do it
    if (pm->ps->weapon == WP_PISTOL)
	{
		BG_FindAmmoForWeapon(pm->ps->weapon, &ammo, &maxClips);
		BG_PackAmmoArray(pm->ps->weapon, &pm->ps->ammo, pm->ps->powerups, ammo, 7);
	}
  ...
  {
    if (maxClips > 0)
    { //hack for pistol not to lose ammo, I know it is bad
      if (pm->ps->weapon != WP_PISTOL)
		clips--;
