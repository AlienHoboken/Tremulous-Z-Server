    ...
    if (client->ps.stats[STAT_STATE] & SS_MEDKIT_ACTIVE)
    {
      ...
      if (remainingStartupTime < 0)
      {
            ...
            && ent->client->medKitHealthToRestore && ent->client->ps.pm_type != PM_DEAD)
        {
          ...
          ent->health++;
			if ( (ent->client->pers.badges[ 18 ] != 1) && (ent->health >= 75) && ent->client->pers.onehp && g_survival.integer)
			{
				G_WinBadge( ent, 18 );
				ent->client->pers.badgeupdate[18] = 1;
				ent->client->pers.badges[18] = 1;
			}
        }
        else
      ...
      else
      {
            ...
            && ent->client->medKitHealthToRestore && ent->client->ps.pm_type != PM_DEAD)
        {
          ...
          if (level.time > client->medKitIncrementTime)
          {
            ent->client->medKitHealthToRestore--;
            ent->health++;
			  if ( (ent->client->pers.badges[ 18 ] != 1) && (ent->health >= 75) && ent->client->pers.onehp)
			  {
				  G_WinBadge( ent, 18 );
				  ent->client->pers.badgeupdate[18] = 1;
				  ent->client->pers.badges[18] = 1;
...
//        LEVEL1_PCLOUD_DMG, 0, MOD_LEVEL1_PCLOUD);
  ...
  while(client->time2000 >= 2000)
  {
    client->time2000 -= 2000;
	 ...
	 //remove this gay shit survival damage . it makes it impossible to not dome camp 
