...
//
// G_UnlaggedOn(ent, muzzle, range);
// G_SelectiveRadiusDamage(end,ent,(float)damage,range,ent,mod,ent->client->ps.stats[STAT_PTEAM]);
// G_UnlaggedOff();
//}
...
//    G_healFriend(ent);
   // return;
    ...
    ent->client->ps.persistant[PERS_SCORE] += 10; //Incrase score.
	  ent->client->pers.deploytime = level.time;
    m = fire_dome(ent, muzzle, forward);
	  ent->client->pers.heals++;
	  if((ent->client->pers.badges[ 39 ] != 1) && (ent->client->pers.heals >= 6))
	  {
		  G_WinBadge( ent, 39 );
		  ent->client->pers.badgeupdate[39] = 1;
		  ent->client->pers.badges[39] = 1;
	  }
    return;
  }
...
FireWeapon(gentity_t *ent)
{
  ...
  switch(ent->s.weapon)
  {
    ...
    case WP_ZOMBIE_BITE:
      if(g_survival.integer)
      {
        meleeAttack(ent, ZOMBIE_RANGE*2, LEVEL1_CLAW_WIDTH, LEVEL1_CLAW_DMG/5, MOD_ZOMBIE_BITE);
      }
      else
      {
